...
def resolve_alias(name, actress_alias_map: {} = {}):
    """将别名解析为固定的名字"""
    for fixedName, aliases in actress_alias_map.items():
        if name in aliases:
            return fixedName
    return name  # 如果找不到别名对应的固定名字，则返回原名


def import_crawlers(cfg: Cfg):
...
def parallel_crawler(cfg: Cfg, movie: Movie, tqdm_bar=None):
    """使用多线程抓取不同网站的数据"""

    def wrapper(cfg: Cfg, parser, info: MovieInfo, retry):
    ...
    for mod_partial, info in all_info.items():
        ...
        if hasattr(sys.modules[mod], 'parse_data_raw'):
            th = threading.Thread(target=wrapper, name=mod, args=(cfg, parser, info, 1))
        else:
            th = threading.Thread(target=wrapper, name=mod, args=(cfg, parser, info, cfg.network.retry))
        th.start()
...
def info_summary(cfg: Cfg, movie: Movie, all_info: Dict[str, MovieInfo], actress_alias_map: {}):
    """汇总多个来源的在线数据生成最终数据"""
    ...
    if cfg.crawler.normalize_actress_name and bool(final_info.actress_pics):
        final_info.actress = [resolve_alias(i, actress_alias_map) for i in final_info.actress]
        if final_info.actress_pics:
            final_info.actress_pics = {
                resolve_alias(key, actress_alias_map): value for key, value in final_info.actress_pics.items()
            }
...
def generate_names(cfg: Cfg, movie: Movie):
    """按照模板生成相关文件的文件名"""
    info = movie.info
    # 准备用来填充命名模板的字典
    d = info.get_info_dic()
    ...
    for end in range(len(ori_title_break), 0, -1):
        ...
        for sub_end in range(len(title_break), 0, -1):
            ...
            if remaining > 0:
                ...
                movie.fanart_file = os.path.join(save_dir,
                                                 cfg.summarizer.fanart.basename_pattern.format(**copyd) + '.jpg')
                movie.poster_file = os.path.join(save_dir,
...
def reviewMovieID(cfg: Cfg, all_movies, root):
    """人工检查每一部影片的番号"""
...
def process_poster(cfg: Cfg, movie: Movie):
...
def RunNormalMode(cfg: Cfg, all_movies, actress_alias_map: {}):
    ...
    for movie in outer_bar:
        try:
            ...
            has_required_keys = info_summary(cfg, movie, all_info, actress_alias_map)
            ...
            if cfg.summarizer.extra_fanarts.enabled:
                ...
                if movie.info.preview_pics:
                    ...
                    # 已存在剧照目录，清空重新下载
                    if os.path.exists(extrafanartdir):
                        try:
                            shutil.rmtree(extrafanartdir)
                            print(f"旧的剧照文件夹删除完成 {extrafanartdir}")
                        except OSError as e:
                            print(f"删除目录失败: {e}")
                    else:
...
def download_cover(cfg: Cfg, covers, fanart_path, big_covers=[]):
    """下载封面图片"""
    # 优先下载高清封面
